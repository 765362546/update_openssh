...
#获取端口信息
S_PORT=$(grep '^Port' /etc/ssh/sshd_config|awk '{print $2}')
S_PORT=${S_PORT:-22}
echo "当前sshd端口为${S_PORT}"
...
echo "Port ${S_PORT}" >> /etc/ssh/sshd_config
...
cp -rf $BAK_PATH/backup_$DATE/openssh/etc/pam.d /etc/pam.d/sshd
